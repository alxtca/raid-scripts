...
def main():
    if (errorhandling.gameReset()  == 'skip'):
        return
    file_path = f'./textfiles/hydraclash.txt'
    print("starting hydra clash extraction", file_path)
...
def _init():
    ...
    shared.clickWrap('hydra_clanboss')

    # clicking chimera clash -> battle - ends clash (no need to press the button)
    # should click back and forth to be sure 
    shared.clickWrapWithSkip('hydra_clash_end')
    shared.clickWrapWithSkip('chimera_claim_chest', 1) #TODO: screenshot of hydra clash end
    shared.clickWrapWithSkip('chimera_claim_chest_confirm', 1)
    time.sleep(1)
    shared.clickWrapWithSkip('hydra_clash_battles')
    time.sleep(1)
    shared.clickWrapWithSkip('hydra_clash_end')
    time.sleep(1)
...
def __ready_to_run_clash(file_path, current_time=None):
    try:
        with open(file_path, 'r') as file:
            ...
            # Check if it’s Wednesday, after 16:00, and hasn’t run today
            did_not_run_today = executed_last_time.date() != current_datetime.date()
            is_friday = current_datetime.weekday() == 2  # Monday=0 ... Friday=4
            is_after_15 = current_datetime.hour >= 16

            print("did_not_run_today ", did_not_run_today)
            print("is_friday ", is_friday)
            print("is_after_15 ", is_after_15)
            print("current_datetime.weekday() ", current_datetime.weekday())
...
def getPlayerChimeraDamage(index):
    ...
    if player_info:
        ...
        left = x+369
        top = y+379 # because of stupid plarium UI, some numbers are 7 pixel above at 380. But then lower numbers wont be read.
        width = 55
        height = 13
        ...
        # some kind of failsafe if reading is too high - can't be above 150.000 M
        # if (converted > 4300):
...
def sendToDiscord(clan_data):
    ...
    with open(chart_file, "rb") as f:
        response = requests.post(
            ...
            data={"content": f"Hydra clash — {today}"}
        )
